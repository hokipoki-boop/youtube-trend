...
export default function App() {
  ...
  async function fetchVideos(key, reg, type) {
    ...
    try {
      ...
      // Shorts: 검색 API로 별도 가져오기
      if (type === "shorts") {
        const searchRes = await fetch(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoDuration=short&q=%23Shorts&regionCode=${reg}&order=viewCount&maxResults=20&key=${key}`
        );
